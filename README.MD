...
calibration/ - датасет для калибровки агентов (см. дополнительные замечания)
...
## Использование иных LLM

По умолчанию фреймворк использует YandexGPT посредством обращения через API Yandex Cloud. Данный вариант реализован в виде класса YandexModel, наследующего от класса Model. При необходимости использования иной модели (например, через OpenRouter) достаточно создать соответствующий простой класс, также наследующий от Model (см. model.py).

## Калибровка агентов

При создании аватаров в целях повышения точности имитации поведения реальных респондентов вам может понадобиться провести калибровку агентов, создаваемых на основе разных LLM (проверку соответствия их реакции на различные тексты), на небольшом датасете, состоящем из текстовых файлов. Пример такого датасета находится в папке calibration. 

Для этой цели удобно использовать метод calibrate класса Agent:

``` python
calibration_results = new_agent.calibrate(calibration_path="calibration", max_tries=3)
print(calibration_results)
```

Кроме того, можно провести калибровку всех агентов фокус-группы:

``` python
calibration_results = new_focus_group.calibrate_all(calibration_path="calibration", max_tries=3)
print(calibration_results)
```
